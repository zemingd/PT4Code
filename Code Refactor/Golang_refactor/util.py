...
def get_random_type_name_and_value_statment_golang():
    datatype = random.choice(
        'byte,int16,int,float32,float64,bool,rune,string'.split(','))
    ...
    mutant = ' var ' + str(var_name) + ' ' + ' struct{} ' + '\n\t' + '_ = ' + str(var_name)
    return mutant
...
def get_branch_if_else_mutant():
    mutant = 'if ('+get_random_false_stmt()+') {' + \
        get_random_type_name_and_value_statment_golang() + \
        '}' + \
        'else{' + \
        get_random_type_name_and_value_statment_golang() + \
        '}'
...
def get_branch_for_imitate_while_mutant():
    mutant = 'for ('+get_random_false_stmt()+') { \n'  + \
        get_random_type_name_and_value_statment_golang() + \
        '\n }'
    return mutant
