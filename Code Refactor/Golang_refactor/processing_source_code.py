...
def word_synonym_replacement_golang( word):
    if len(word) <= 3:
        return word + '_new'
    word_set = wordninja.split(word)
    while True:
...
def extract_method_name( string):
    mainIndex = string.find('main(')
    match_ret = re.search('\w+\s*\(', string[mainIndex + 5:])
    pattern = re.compile(r'\bfunc\s+(\w+)')
...
def extract_local_variable( string):
    ...
    for line in statement_list:
        match_ret = re.search(r'\bvar\s+((?:[A-Za-z_]\w*,\s*)*[A-Za-z_]\w*)\s*(\w[\w.]*)\s*(=\s*.*)?', line)
        if match_ret:
            var_definition = match_ret.group()
            local_var_list.append(var_definition.split(' ')[1].replace(',', ''))
