...
func (st segmentTree) query(start, stop int) int {
	...
	/*
		result := 0
		q := make([][3]int, 1)
		q = append(q, [3]int{0, 0, st.offset + 1})
		for len(q) != 0 {
			index := q[0][0]
			left := q[0][1]
			right := q[0][2]
			...
			if start <= left && right <= stop {
				result = gcd(result, st.data[index])
				continue
			}
			q = append(q, [3]int{index*2 + 1, left, (left + right) / 2})
			q = append(q, [3]int{index*2 + 2, (left + right) / 2, right})
		}
		return result
	*/
}
