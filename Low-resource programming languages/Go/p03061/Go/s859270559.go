...
func main() {
	N := getInt()
	A := getIntArray(N)
	leftGcdList := make([]int, N)
	rightGcdList := make([]int, N)

	leftGcdList[0] = A[0]
	rightGcdList[N-1] = A[N-1]
	for i := 1; i < N; i++ {
		leftGcdList[i] = calcGcd(leftGcdList[i-1], A[i])
		rightGcdList[N-1-i] = calcGcd(rightGcdList[N-i], A[N-1-i])
	}
	...
	for i := 0; i < N; i++ {
		...
		if i == 0 {
			...
			gcd = calcGcd(leftGcdList[i-1], rightGcdList[i+1])
		}
