...
func main() {
	...
	for i := int64(0); i < N-1; i++ {
		distance[i] = abs(A[i+1] - A[i])
		distanceSum += distance[i]
		if maxIndex < distance[i] {
			maxIndex = i
		}
	}
	distance[N-1] = abs(K - A[N-1] + A[0])
	distanceSum += distance[N-1]
	if maxIndex < distance[N-1] {
		maxIndex = N - 1
	}
	fmt.Println(distanceSum - distance[maxIndex])
}
