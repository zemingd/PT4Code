...
func main() {
	...
	/*
		    // 解法1
				...
				var total int64
				total = ((sum % mod) * (sum % mod)) % mod
				total -= mms
				if total < 0 {
					total += mod
				}
				// 2の逆元を求める
				var gyaku2 int64 = powMod64(2, mod-2, mod)

				total = total * gyaku2 % mod
				fmt.Println(total)
	*/
}
...
func powMod64(a, n, mod int64) int64 {
	if n == 0 {
		return 1 % mod
	}
	if n == 1 {
	...
	if n%2 == 1 {
		return powMod64((a*a)%mod, n/2, mod) * a % mod
	}
	return powMod64((a*a)%mod, n/2, mod)
}
