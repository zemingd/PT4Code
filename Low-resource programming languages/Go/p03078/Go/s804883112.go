...
func (i1 Item) Less(i2 interface{}) bool {
	return i1.val > i2.(Item).val
}
...
func main() {
	...
	used := NewSet()
	q := NewPriorityQueue()
	q.Push(Item{A[X-1] + B[Y-1] + C[Z-1], X - 1, Y - 1, Z - 1})
	used.Add(Param{X - 1, Y - 1, Z - 1})
	for i := 0; i < K; i++ {
		t := q.Pop().(Item)
		if t.x > 0 && !used.Contains(Param{t.x - 1, t.y, t.z}) {
			q.Push(Item{A[t.x-1] + B[t.y] + C[t.z], t.x - 1, t.y, t.z})
			used.Add(Param{t.x - 1, t.y, t.z})
		}
		if t.y > 0 && !used.Contains(Param{t.x, t.y - 1, t.z}) {
			q.Push(Item{A[t.x] + B[t.y-1] + C[t.z], t.x, t.y - 1, t.z})
			used.Add(Param{t.x, t.y - 1, t.z})
		}
		if t.z > 0 && !used.Contains(Param{t.x, t.y, t.z - 1}) {
			q.Push(Item{A[t.x] + B[t.y] + C[t.z-1], t.x, t.y, t.z - 1})
			used.Add(Param{t.x, t.y, t.z - 1})
		}
		printf("%d\n", t.val)
	}
}
