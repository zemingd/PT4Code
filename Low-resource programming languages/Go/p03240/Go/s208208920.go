...
func main() {
	...
	for i := 0; i < N; i++ {
		xMap[X[i]] = append(xMap[X[i]], H[i])
		yMap[Y[i]] = append(yMap[Y[i]], H[i])
		M[Y[i]][X[i]] = H[i]
	}

	xRange := getRange(xMap)
	yRange := getRange(yMap)
		...
		for x := xRange[0]; x <= xRange[1]; x++ {
			...
			for ty := y-2; ty <= y+2; ty++ {
				for tx := x-2; tx <= x+2; tx++ {
					...
					//fmt.Println("x:", x, "y:", y, "tx:", tx, "ty:", ty)

					if h == 0 {
						h = M[ty][tx] + abs(x - tx) + abs(y - ty)
					} else {
						if h != M[ty][tx] + abs(x - tx) + abs(y - ty) {
							flag = false
							break
						}
					}
