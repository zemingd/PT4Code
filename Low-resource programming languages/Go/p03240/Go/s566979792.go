...
func solve(stdin io.Reader, stdout io.Writer) {
	...
	for i := 0; i < n; i++ {
		r.Scan(true)
		ln[i] = line{r.nextInt(), r.nextInt(), r.nextInt()}
		if ln[i].h >= 0 && (!isSet || m > ln[i].h) {
			m = ln[i].h
		}
	}
	...
	cur := line{}
loop:
	for cur.y = 0; cur.y <= 100; cur.y++ {
		for cur.x = 0; cur.x <= 100; cur.x++ {
			end := m + 100
		nextConfirm:
			for cur.h = m; cur.h <= end; cur.h++ {
				for i := range ln {
					if ln[i].h != maxInt(cur.h-absInt(ln[i].x-cur.x)-absInt(ln[i].y-cur.y), 0) {
						continue nextConfirm
					}
				}
				break loop
	...
	fmt.Fprintln(stdout, cur.x, cur.y, cur.h)
}
