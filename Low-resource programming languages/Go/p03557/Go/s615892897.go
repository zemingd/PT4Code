...
func main() {
	...
	var count int
	for i := 0; i < n; i++ {
		aCount := SearchLeft(a, b[i])+1
		cCount := n - SearchRight(c, b[i])
		count += aCount * cCount
	}
...
// SearchLeft returns the index of the highest value in the values lower than x
func SearchLeft(a []int, x int) int {
	return sort.SearchInts(a, x)-1
}

// SearchRight returns the index of the lowest value in the values higher than x
func SearchRight(a []int, x int) int {
