...
func main() {
	...
	for i := 0; i < len(bingo); i++ {
		bingo[i] = make([]int, 3)
		bingob[i] = make([]bool, 3)
	}
	for i := 0; i < 3; i++ {
		for j := 0; j < 3; j++ {
			bingo[i][j] = eGetInt(scanner)
		}
	}
	n := eGetInt(scanner)
	for i := 0; i < n; i++ {
	...
	//横
	for i := 0; i < 3; i++ {
		if bingob[i][0] && bingob[i][1] && bingob[i][2] {
			fmt.Println("Yes")
			return
		}
	}
	//縦
	for i := 0; i < 3; i++ {
		if bingob[0][i] && bingob[1][i] && bingob[2][i] {
			fmt.Println("Yes")
			return
		}
	}
	//左上から右下
	if bingob[0][0] && bingob[1][1] && bingob[2][2] {
		fmt.Println("Yes")
		return
	}
	//左下から右上
	if bingob[2][0] && bingob[1][1] && bingob[0][2] {
		fmt.Println("Yes")
		return
	}
	fmt.Println("No")
