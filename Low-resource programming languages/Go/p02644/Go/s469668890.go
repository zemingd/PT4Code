...
func NewDijkstraSolve(
	vinf, vinit V, Less func(l, r V) bool, GenNextV func(cv *Vertex, e Edge) V,
) *DijkstraSolver {
	ds := new(DijkstraSolver)

	__less = Less

	ds.vinf, ds.vinit = vinf, vinit
...
func (ds *DijkstraSolver) InitStartPoint(S []int, dp []V, colors []int) *VertexPQ {
	pq := NewVertexPQ()

	for _, sid := range S {
		pq.push(&Vertex{id: sid, v: ds.vinit})
		dp[sid] = ds.vinit
		colors[sid] = GRAY
	}
...
func (ds *DijkstraSolver) Dijkstra(S []int, n int, AG [][]Edge) []V {
	// initialize data
