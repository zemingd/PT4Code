...
func main() {
	...
	for pq.Len() > 0 {
		...
		for _, to := range G[pop.id] {
			...
			nv := V{num: dp[pop.id].num, nokori: dp[pop.id].nokori}
			if prevd != nextd {
				// 方向転換
				nv.num++
				nv.nokori = k
...
func (pq VertexPQ) Less(i, j int) bool {
	...
	// }
	if pq[i].v.num < pq[j].v.num {
		return true
	} else if pq[i].v.num > pq[j].v.num {
		return false
	} else {
		return pq[i].v.nokori > pq[j].v.nokori
	}
} // <: ASC, >: DESC
