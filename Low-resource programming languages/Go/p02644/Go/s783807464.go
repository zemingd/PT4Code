...
func main() {
	...
	pq := NewPriorityQueue(func(p, q PriorityQueueElement) bool {
		return p.d < q.d
	})
	push := func(x, y, d int) {
		pq.Push(PriorityQueueElement{x, y, d})
	}
	pop := func() (int, int, int) {
		res := pq.Pop()
	...
	for pq.Len() != 0 {
		...
		for dir := range dx {
			nd := d[x][y] + 1
			for k, nx, ny := 1, x+dx[dir], y+dy[dir]; k <= K && 0 <= nx && nx < H && 0 <= ny && ny < W && c[nx][ny] == '.'; k, nx, ny = k+1, nx+dx[dir], ny+dy[dir] {
				if d[nx][ny] < nd {
					break
				}
				if d[nx][ny] == nd {
...
func NewPriorityQueue(cmp func(x, y PriorityQueueElement) bool) *PriorityQueue {
	pq := &PriorityQueue{
		a: make([]PriorityQueueElement, 0),
	}
	pq.cmp = func(i, j int) bool {
		return cmp(pq.a[i], pq.a[j])
	}
	return pq
}
...
func (pq *PriorityQueue) lch(i int) int {
	return i*2 + 1
}

func (pq *PriorityQueue) rch(i int) int {
	return i*2 + 2
}
...
func (pq *PriorityQueue) Push(v PriorityQueueElement) {
	pq.a = append(pq.a, v)
	n := len(pq.a) - 1
	for i, p := n, pq.parent(n); 0 != i && !pq.cmp(p, i); i, p = p, pq.parent(p) {
		pq.swap(i, p)
	}
}

func (pq *PriorityQueue) Top() PriorityQueueElement {
	return pq.a[0]
}

func (pq *PriorityQueue) Pop() PriorityQueueElement {
	n := len(pq.a) - 1
	pq.a[0], pq.a[n] = pq.a[n], pq.a[0]
	res := pq.a[n]
	pq.a = pq.a[:n]
	p := 0
	for {
