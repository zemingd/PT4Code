...
func main() {
	...
	for index := 0; index < N; index++ {
		table[index] = key{oneInt(), oneInt()}
	}

	for index := 0; index < N; index++ {
		for weight := 1; weight <= W; weight++ {
			var temp int
			if weight-table[index].w >= 0 {
				temp = dp[key{index, weight - table[index].w}] + table[index].v
				dp[key{index + 1, weight}] = max(dp[key{index + 1, weight}], temp)
            } else {
              			dp[key{index + 1, weight}] = dp[key{index, weight}]

            }
		}
	}
	fmt.Print(dp[key{N, W}])
}
