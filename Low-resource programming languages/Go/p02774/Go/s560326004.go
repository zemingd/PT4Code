...
func solve(N, K int, A []int) int {
	...
	k := LowerBound(0, maxv, func(d int) bool {
		d -= i9 * i9
		var count int
		for i, v := range A {
			var C []int
			...
			//eprintln(v, "C", C)
			x := LowerBound(0, len(C), func(j int) bool {
				//eprintln(d, v, C[j], v*C[j], d <= v*C[j])
				return d < v*C[j]
			})
			//eprintln(x)
			//eprintln("d", d, "v*C[j]", v, C[j], "=", v*C[j])
			//eprintln("d", d, "j", j, "C[j]", C[j])
			count += x
		}
		//eprintln("d", d, "->", "count", count, "K", K)
		return K <= count
	})
...
func LowerBound(l, r int, fn func(i int) bool) int {
	for l < r {
		m := (l + r) >> 1
		if fn(m) {
			r = m
