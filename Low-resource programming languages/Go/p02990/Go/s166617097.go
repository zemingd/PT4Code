...
func solve(n, k int) {
	...
	// maxInt := max(n-k, k)
	for i := 1; i <= n; i++ {
		pattern[i] = pattern[i-1] * i % Mod
	}
	DEBUG = false
	debug(pattern)
	for i := 1; i <= k; i++ {
		bluePattern := nCk(k-1, i-1) //pattern[k-1] / (pattern[i-1] * pattern[k-i]) // (k-1)C(i-1) = (k-1)! / (i-1)! * (k-1-i)!
		debug("bluePattern", bluePattern)
		// debug("pattern", pattern[n-k+i]%Mod/(pattern[n-k+1-i]%Mod*pattern[i]%Mod))
		debug(n-k+1, "C", i, "=", nCk(n-k+1, i))
		answer := bluePattern * nCk(n-k+1, i) // bluePattern * pattern[n-k+i] % Mod / (pattern[n-k+1-i] % Mod * pattern[i] % Mod)
		println(answer)
	}
}
...
func nCk(n, k int) int {
	...
	return moddiv(moddiv(pattern[n], pattern[n-k]), pattern[k])
}
