...
func main() {
	...
	if w%2 == 0 && h%2 == 0 && w/2 == x && y == h/2 {
		fmt.Println(float64(h*w)/2.0, 1)
		return
	}
	...
	ww, hh, xx, yy := float64(w), float64(h), float64(x), float64(y)

	// y = y で切るとき
	s1 := math.Min(ww*yy, ww*(hh-yy))
	// x = x で切るとき
	s2 := math.Min(hh*xx, hh*(ww-xx))

	fmt.Println(math.Max(s1, s2), 0)
}
