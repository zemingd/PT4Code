...
func solve(k int) int {
	...
	mod = k
	pow10Map = make(map[int]int)
	pow10Map[0] = 1
	remainder := 0
	// return -1
	for i := 0; ; i++ {
		remainder += 7 * pow10(i) % k
		remainder %= k
		// debug(7*pow10(i), i, remainder)
		if remainder%k == 0 {
			return i + 1
		}
	}
	...
	// for i := 0; ; i++ {
	// 	s += sevenStr
	// 	if sevenInt, _ := strconv.ParseInt(s, 10, 64); sevenInt%int64(k) == 0 {
	// 		return i + 1
	// 	} else {
	// 		debug(sevenInt, i)
	// 	}
