...
func main() {
	...
	pointList := make([]int, Q)
	arrayList := make([][]int, Q)

	for i := 0; i < Q; i++ {
		arrayList[i] = make([]int, N)
		for j := 0; j < N; j++ {
			arrayList[i][j] = -1
		}
	}
	...
	for i := 0; i < Q; i++ {
		for ni := 0; ni <= nums[i].A; ni++ {
			arrayList[i][ni] = 0
		}

		arrayList[i][nums[i].B] = nums[i].C
		pointList[i] += nums[i].D
	}
	...
	for i := 0; i < Q; i++ {
		for ai := 0; ai < Q; ai++ {
			...
			if max(arrayList[ai][:nums[i].A+1]...) + nums[i].C > M-1 {
				continue
			}

			if max(arrayList[ai][:nums[i].A+1]...) + nums[i].C == arrayList[ai][nums[i].B] {
				arrayList[ai][nums[i].A] = max(arrayList[ai][:nums[i].A+1]...)
				pointList[ai] += nums[i].D
			} else if max(arrayList[ai][:nums[i].A+1]...) + nums[i].C < arrayList[ai][nums[i].B] && arrayList[ai][nums[i].A] == -1 {
				arrayList[ai][nums[i].A] = arrayList[ai][nums[i].B] - nums[i].C
				pointList[ai] += nums[i].D
			} else if arrayList[ai][nums[i].B] == -1 {
				arrayList[ai][nums[i].A] = max(arrayList[ai][:nums[i].A+1]...)
				arrayList[ai][nums[i].B] = arrayList[ai][nums[i].A] + nums[i].C
				pointList[ai] += nums[i].D
			}
		}
	}
