...
func main() {
	...
	combinations := generateCombinations(1, m, n)
...
func generateCombinations(min, max, n int) []SeqA {
	if n == 0 {
		return []SeqA{make(SeqA, 0)}
	}

	combinations := make([]SeqA, 0)
	for i := min; i <= max; i++ {
		restCombinations := generateCombinations(min, i, n-1)
		for _, combi := range restCombinations {
			combinations = append(combinations, append(combi, i))
		}
	}
	return combinations
}
