...
func main() {
	...
	patterns := genPatterns(N, M, []int{})
	//for i := 0; i < len(patterns); i++ {
	//	fmt.Println(patterns[i])
	//}

	points := make([]int, len(patterns))
	for pi, pattern := range patterns {
		for i := 0; i < Q; i++ {
			if pattern[nums[i].B] - pattern[nums[i].A] == nums[i].C {
				points[pi] += nums[i].D
			}
		}
	}
...
func genPatterns(n, m int, pattern []int) [][]int {
	if len(pattern) == n {
		return [][]int{pattern}
	}
	...
	start := 1
	if len(pattern) != 0 {
		start = pattern[len(pattern)-1]
	}
	for i := start; i <= m; i++ {
		patterns = append(patterns, genPatterns(n, m, append(pattern, i))...)
	}
	return patterns
}
