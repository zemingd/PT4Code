...
func main() {
	...
	println(calc5x(N, 5) + calc10(N, 10))
}

func calc5x(N, x int) int {
	a := calc5(N, x*10)
	if a == 0 {
		return 0
	}
	return a + calc5x(N, x*5)
}

func calc5(N, d int) int {
	if N/d == 0 {
		return 0
	}
	//dbg("calc5", N, d, N/d, N/(2*d), N/d-N/(2*d))
	return N/d - N/(2*d) + calc5(N, d*10)
}

func calc10(N, d int) int {
	if N/d == 0 {
		return 0
	}
	//dbg("calc10", N, d, N/d)
	return N/d + calc10(N, d*10)
}
