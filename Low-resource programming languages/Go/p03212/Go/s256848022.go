...
func main() {
	...
	for i := 3; i <= length; i++ {
		...
		for _, pattern := range patterns {
			if !strings.Contains(pattern, "3") || !strings.Contains(pattern, "5") || !strings.Contains(pattern, "7") {
				continue
			}

			number, _ := strconv.Atoi(pattern)
			if number <= N {
				result++
			}
		}
...
func getPatterns(n int) []string {
	patterns := []string{"3", "5", "7"}

	for i := 0; i < n-1; i++ {
		var tmp []string
		for _, pattern := range patterns {
			tmp = append(tmp, pattern + "3")
			tmp = append(tmp, pattern + "5")
			tmp = append(tmp, pattern + "7")
		}
		patterns = tmp
	}
