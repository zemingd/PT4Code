...
func solve(n, k int, a []int) int {
	...
	for i := 0; i+1 < k; i += 2 {
		if minusIndex+2 >= underZero {
			product *= a[plusIndex]
			product %= Mod
			product *= a[plusIndex-1]
			product %= Mod
			plusIndex -= 2
		} else if plusIndex+2 < underZero {
			product *= -1 * a[minusIndex]
			product %= Mod
			product *= -1 * a[minusIndex+1]
			product %= Mod
			minusIndex += 2
		} else if (a[minusIndex]+a[minusIndex+1])*-1 >= a[plusIndex]+a[plusIndex-1] {
			product *= -1 * a[minusIndex]
			product %= Mod
			product *= -1 * a[minusIndex+1]
			product %= Mod
	...
	debug("mi", minusIndex, underZero, "pi", plusIndex)
