...
func solve() {
	...
	if N - zero < K {
		...
		for i := 0 ; i < N; i++ {
			ans = (ans * (A[i] + magic)) % magic
		}
	} else if K % 2 == 1 {
		if plus >= 1 {
			Logf("find max positive\n")
			ind := 0
			pind := 0
			nind := 0
			for {
				if K - ind >= 2 {
					if plus - pind >= 2 && minus - nind >= 2 {
						if A[pind] * A[pind+1] > A[N-1-nind] * A[N-2-nind] { // choose 2 positives
							// ans = (ans * (A[pind] + magic)) % magic
							// ans = (ans * (A[pind+1] + magic)) % magic
							pind += 2
						} else { // choose 2 negatives
							// ans = (ans * (A[N-1-nind] + magic)) % magic
							// ans = (ans * (A[N-2-nind] + magic)) % magic
							nind += 2
						}
					} else if plus - pind >= 2 {
						// ans = (ans * (A[pind] + magic)) % magic
			...
			Logf("find min negative\n")
			for i := 0; i < K; i++ {
				ans = (ans * (A[i] + magic)) % magic
			}
		}
	} else { // K is even
		Logf("find max positive 2\n")
		ind := 0
		pind := 0
		nind := 0
		for {
