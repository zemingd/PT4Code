...
func main() {
	scanner := bufio_NewScanner(os.Stdin)
	const initialBufSize = 4096
	const maxBufSize = 1000000
	scanner_Buffer(make([]byte, initialBufSize), maxBufSize)
	scanner_Split(bufio.ScanWords)
	var N int64
	scanner_Scan()
	N, _ = strconv_ParseInt(scanner_Text(), 10, 64)
	fmt_Println(solve(N))
}
