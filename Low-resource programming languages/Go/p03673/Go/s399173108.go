...
// 毎回appendするとTLEになってしまうのでappendの回数を減らすようにする
...
func main() {
	...
	length := n / 2
	bfront := make([]int, length)
	bback := make([]int, length)
	for i := 0; i < n/2; i++ {
		switch n % 2 {
		case 0:
			bback[i] = nextInt()
			bfront[len(bfront)-1-i] = nextInt()
		case 1:
			bfront[len(bback)-1-i] = nextInt()
			bback[i] = nextInt()
		}

	}
	// fmt.Println(bfront, bback)
	if n%2 == 1 {
		bfront = append([]int{nextInt()}, bfront...)
	}
	b := append(bfront, bback...)
