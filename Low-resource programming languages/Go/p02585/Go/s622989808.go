...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		location := i
		for j := 0; j < N; j++ {
			if _, e := check[P[location]]; e {
				cycle[i] = j
				break
			}
			check[P[location]] = true
			location = P[location]
		}
	}
	// fmt.Println(cycle)
	...
	for i := 0; i < N; i++ {
		...
		for j := 0; j < l; j++ {
			...
			memo[i][j+1] = memo[i][j] + C[location]
			location = P[location]
			max = MaxOf(max, memo[i][j+1])
		}
	}
	// fmt.Println(memo)
	if max <= 0 || K <= N {
	...
	for i := 0; i < N; i++ {
		...
		for j := 0; j < K%l; j++ {
			// fmt.Println(summax)
			summax = MaxOf(summax, start+memo[i][j+1])
		}
		max = MaxOf(max, summax)
	}
	fmt.Println(max)
	// for i := 0; i < N; i++ {
	// 	memo[i] = make([]int, N+1)
	// 	location := i
	// 	for j := 0; j < MinOf(N, K); j++ {
	// 		memo[i][j+1] += memo[i][j] + C[location]
	// 		// maxlist[i] = MaxOf(maxlist[i], memo[i][j+1])
	// 		// fmt.Println(memo[i], C[location], location)
	// 		location = P[location]
	// 		max = MaxOf(max, memo[i][j+1])
	// 		// fmt.Println(max)
	// 	}
	...
	// 		summax := (K / N) * memo[i][N]
	// 		for j := 0; j < K%N; j++ {
	// 			summax = MaxOf(summax, summax+memo[i][j+1])
	// 			fmt.Println(summax)
	// 		}
	// 		max = MaxOf(max, summax)
	// 	}
