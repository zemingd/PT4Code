...
func solve(n, k int, p, c []int) int {
	...
	debug(routes)
	maxPoint := -10000000000000
	for _, r := range routes {
		point := 0
		if r.point > 0 && k > len(r.trace) {
			point += k / len(r.trace) * r.point
			k %= len(r.trace)
		}
		maxInRoute := -10000000000000
		for start := 0; start < len(r.trace); start++ {
			looped := false
			for end := start; ; end++ {
				debug(start, end, "k", k)
				if end == len(r.trace) {
				...
				if start <= end && end-start > k {
					continue
				} else if start > end && len(r.trace)-start+end > k {
					continue
				}
