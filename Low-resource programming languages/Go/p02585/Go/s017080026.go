...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if k <= loopFin || log[loopFin]-log[loopStart] <= 0 {
			...
			z += (log[loopFin] - log[loopStart]) * (((k - loopStart) / dif) - 1)
			k = (k-loopStart)%dif + dif
			if (log[loopFin]-log[loopStart])*(((k-loopStart)/dif)-1) > 0 {
				y = larger(y, z)
			}
			for j := 1; j <= dif; j++ {
				log = append(log, log[len(log)-1]+log[loopStart+j]-log[loopStart+j-1])
			}
			for j := 1; j <= k; j++ {
