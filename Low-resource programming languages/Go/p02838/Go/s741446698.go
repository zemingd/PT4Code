...
func decimalToBin(decNum int) (bin int) {
	for ct := 1; decNum != 0; ct *= 10 {
		bin += (decNum % 2) * ct
		decNum /= 2
	}
	return
...
func solve() {
	...
	countBit := make([]int, 65)
	for i := range countBit {
		for j := range as {
			if ((as[j] >> uint(i)) & 1) == 1 {
				countBit[i]++
			}
	...
	ans := 0
	for i, ct := range countBit {
		x := ModPow(2, i, mod) * (ct * (n - ct)) % mod
		x %= mod
		ans = (ans + x) % mod
	}
	pln(ans)
