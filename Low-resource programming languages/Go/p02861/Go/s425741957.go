...
func main() {
	...
	for i := 0; i < N; i++ {
		xy := sc.nextInts()
		aPair[i] = Pair{first: xy[0], second: xy[1]}
	}
	var ans float64
	for i := 0; i < N; i++ {
		for j := 0; j < N; j++ {
			...
			p1 := aPair[i]
			p2 := aPair[j]
			x1y1 := (p1.first - p2.first) * (p1.first - p2.first)
			x2y2 := (p1.second - p2.second) * (p1.second - p2.second)
			ans += math.Sqrt(float64(x1y1) + float64(x2y2))
		}
	}
