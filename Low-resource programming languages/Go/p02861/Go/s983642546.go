...
func main() {
	...
	count++
	for i := int64(0); NextPermutation(sort.Interface(A)); i++ {
		for j := 0; j < len(A)-1; j++ {
			sum += calc(A[j], A[j+1])
		}
		count++
...
func calc(p1 pair, p2 pair) float64 {
	x1 := p1.p1.(int64)
	y1 := p1.p2.(int64)
	x2 := p2.p1.(int64)
	y2 := p2.p2.(int64)
	return math.Sqrt(math.Pow(float64(x2-x1), 2.0) + math.Pow(float64(y2-y1), 2))
}
