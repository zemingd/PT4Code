...
func main() {
	...
	arr := make([][]int, N)
	for i := 0; i < N; i++ {
		var index, Max int = 1e9, 0

		for j := 0; j < len(arr); j++ {
			if len(arr[j]) == 0 {
				if index == 1e9 {
					index = j
					arr[index] = make([]int, 1)
				}
				break
			} else if Max < arr[j][len(arr[j])-1] && arr[j][len(arr[j])-1] < A[i] {
				Max = arr[j][len(arr[j])-1]
				index = j
			}
		}
		arr[index][max(len(arr[index])-1, 0)] = A[i]
	}
	for i := 0; i < N; i++ {
		if 0 < len(arr[i]) {
			ans++
