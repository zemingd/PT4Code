...
func Eln(s ...interface{}) {
	fmt.Fprintln(os.Stderr, s...)
}

func Ef(f string, s ...interface{}) {
...
func Mset(s string) map[rune]struct{} {
	r := make(map[rune]struct{}, len(s))
...
func Mcount(s string) map[rune]int {
	r := make(map[rune]int, len(s))
...
func MapValuesRune(m map[rune]int) (r []int) {
	for _, v := range m {
		r = append(r, v)
	}
	return
...
func EqualArrayInt(a, b []int) bool {
	if len(a) != len(b) {
		return false
	}
	for i := 0; i < len(a); i++ {
