...
func solve() {
	...
	result := count(par, N+1)
	fmt.Println(max_list(result))
}

func union_find(N int) []int {
	par := make([]int, N)
	for i := 0; i < N; i++ {
		par[i] = i
	}
...
func root(par []int, x int) int {
	// データxが木の根の場合は,xを返す
	// データxが木の根でない場合はparで親を再帰的にたぐり、根を返す
	if par[x] == x {
		return x
	} else {
...
func unite(par []int, x, y int) []int {
	// xの根のrxとyの根のryが同じでないならばxの根のrxを根ryにつける
	rx := root(par, x)
	ry := root(par, y)
	if rx != ry {
		par[rx] = ry
