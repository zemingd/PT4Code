...
func (slice numbers) Len() int {
	return len(slice)
}

func (slice numbers) Less(i, j int) bool {
	return slice[i].cnt > slice[j].cnt
}

func (slice numbers) Swap(i, j int) {
	slice[i], slice[j] = slice[j], slice[i]
}
...
func main() {
	...
	for k, v := range m {
		num = append(num, number{k, v})
	}
	threshold := n / 2
	sort.Sort(num)
	var change int64
	if len(num) == 1 {
		change += abs(threshold - num[0].cnt)
	} else {
		for i := 0; i < 2; i++ {
			if threshold < num[i].cnt {
				change += abs(threshold - num[i].cnt)
			}
		}
		for i := 2; i < len(num); i++ {
			change += num[i].cnt
