...
func main() {
	...
	cnt1 := numMap1[maxCntNum]
	cnt2 := numMap2[maxCntNum]

	targetNum1 := 0
	targetNum2 := 0
	if cnt1 <= cnt2 {
		targetNum2 = maxCntNum
	} else {
		targetNum1 = maxCntNum
	}

	changeCnt1 := 0
	if len(numMap1) != 1 {
		if targetNum1 != 0 {
			cnt := numMap1[targetNum1]
			changeCnt1 = len(numMap1) - cnt
		} else {
			maxCnt := 0
	...
	if len(numMap2) != 1 {
		if targetNum2 != 0 {
			cnt := numMap2[targetNum2]
			changeCnt1 = len(numMap2) - cnt
		} else {
			maxCnt := 0
