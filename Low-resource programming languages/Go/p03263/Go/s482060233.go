...
func main() {
	...
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			...
			} else if j-1 >= 0 && M[i][j-1] != 0 {
				selected[i][j] = 1
				M[i][j]--
				M[i][j-1]++
				result = append(result, []int{i, j, i, j-1})
			} else if i+1 < H && M[i+1][j] != 0 {
				selected[i][j] = 1
				M[i][j]--
				M[i+1][j]++
				result = append(result, []int{i, j, i+1, j})
			} else if j+1 < W && M[i][j+1] != 0 {
				selected[i][j] = 1
				M[i][j]--
				M[i][j+1]++
				result = append(result, []int{i, j, i, j+1})
	...
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			...
			if i-1 >= 0 && M[i-1][j] % 2 == 1 {
				...
				M[i-1][j]++
				result = append(result, []int{i, j, i-1, j})
			} else if j-1 >= 0 && M[i][j-1] % 2 == 1 {
				M[i][j]--
				M[i][j-1]++
				result = append(result, []int{i, j, i, j-1})
			} else if i+1 < H && M[i+1][j] % 2 == 1 {
				M[i][j]--
				M[i+1][j]++
				result = append(result, []int{i, j, i+1, j})
			} else if j+1 < W && M[i][j+1] % 2 == 1 {
				M[i][j]--
				M[i][j+1]++
				result = append(result, []int{i, j, i, j+1})
			}
