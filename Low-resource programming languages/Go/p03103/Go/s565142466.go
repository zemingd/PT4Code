...
func main() {
	...
	drinks := make([]drink, N)
	for i := int64(0); i < N; i++ {
		drinks[i] = drink{readInt(), readInt()}
	}
	for j := len(drinks) - 1; j >= 0; j-- {
		for i := 0; i <= j-1; i++ {
			if drinks[i].cost > drinks[i+1].cost {
				drinks[i], drinks[i+1] = drinks[i+1], drinks[i]
			}
		}
	}
	allcost := int64(0)
	for i := 0; i < len(drinks); i++ {
		if drinks[i].num >= M {
			allcost += (drinks[i].cost) * M
			break
		}
		allcost += drinks[i].cost * drinks[i].num
		M -= drinks[i].num
		if M <= 0 {
			break
		}
