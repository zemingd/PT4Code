...
func main() {
	...
	tArray := make([]string, len(t))
	for i, c := range t {
		tArray[i] = string(c)
	}

	sort.Sort(sort.StringSlice(sArray))
	sort.Sort(sort.Reverse(sort.StringSlice(tArray)))
	...
	if result == "No" && checkedCnt == len(s) {
		if strings.Contains(t, s) && s != t {
			result = "Yes"
		}
	}
