...
func main() {
	...
	comp, cnts := RunLengthEncoding(S)

	sums := make([]int, len(cnts)+1)
	for i := 0; i < len(cnts); i++ {
		sums[i+1] = sums[i] + cnts[i]
	}

	ans := 0
	if comp[0] == '1' {
		for i := 0; i+(2*k+1) < len(sums); i++ {
			ChMax(&ans, sums[i+2*k+1]-sums[i])
		}
		if len(cnts) <= 2*k+1 {
			ChMax(&ans, sums[len(cnts)])
		}
	} else {
		for i := 0; i+2*k < len(sums); i++ {
			ChMax(&ans, sums[i+2*k]-sums[i])
		}
		if len(cnts) <= 2*k {
			ChMax(&ans, sums[len(cnts)])
		}
