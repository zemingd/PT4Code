...
type zeroPart struct {
	start int
	end   int
	sum   int
}

type zeroPartList []zeroPart

func (f zeroPartList) Len() int {
	return len(f)
}

func (f zeroPartList) Less(i, j int) bool {
	return f[i].sum < f[j].sum
}

func (f zeroPartList) Swap(i, j int) {
	f[i], f[j] = f[j], f[i]
}
...
func main() {
	...
	for z := 0; z < k; z++ {
		...
		for i < n {
			...
			zeros = append(zeros, zeroPart{start, i, nones + nzeros})
		}

		sort.Sort(sort.Reverse(zeros))
		for i := zeros[0].start; i < zeros[0].end; i++ {
			si[i] = 1
		}
	}
