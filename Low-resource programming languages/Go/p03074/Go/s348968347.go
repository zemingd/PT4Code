...
func main() {
	...
	seqCnts := make([][]int, N)
	index := 0
	seqCnts[0] = []int{states[0], 1}
	for i := 1; i < N; i++ {
		if states[i-1] != states[i] {
			index++
			seqCnts[index] = []int{states[i], 1}
		} else {
			seqCnts[index][1]++
		}
	}
	seqCnts = seqCnts[:index+1]
	...
	for ; ; {
		...
		targetIndex := 0
		leftIndex := 0
		for i := 1; i < len(seqCnts); i++ {
			if seqCnts[i][0] == 1 {
				cnt := 0
				for j := leftIndex; j <= i; j++ {
					cnt += seqCnts[j][1]
				}
				if maxCnt < cnt {
					maxCnt = cnt
		...
		value := seqCnts[targetIndex][1]
		if targetIndex > 0 {
			value += seqCnts[targetIndex-1][1]
			tmp = seqCnts[:targetIndex-1]
		}
		...
		tmp = append(tmp, []int{1, value})

		if targetIndex < N-1 {
			tmp = append(tmp, seqCnts[targetIndex+2:]...)
		}
		...
		if len(seqCnts) == 1 && seqCnts[0][0] == 1 {
			break
		}
	}

	result := 0
	for i := 0; i < len(seqCnts); i++ {
		if seqCnts[i][0] == 1 && result < seqCnts[i][1] {
			result = seqCnts[i][1]
		}
	}
	fmt.Println(result)
