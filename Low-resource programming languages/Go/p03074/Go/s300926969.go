...
func main() {
	...
	// データの圧縮
	press := make([]int, 0)
	// 先頭が0からの場合は、ダミーの1の圧縮データを入れておく
	if S[0] == '0' {
		press = append(press, 0)
	}
	seq := 0
	for i := 0; i < n; i++ {
		if i == 0 {
	...
	// フラグ管理
	flags := make([]bool, len(press))
	// 先頭が0からの場合でも、ダミーによって最初はtrueから
	// if S[0] == '1' {
	// 	flags[0] = true
	// } else {
	// 	flags[0] = false
	...
	// 0の集まりの個数をカウントして、全部反転できるのならば答えを出して終了しておく
	zeroCount := 0
	...
	sums := make([]int, len(press)+1)
	for i := 0; i < len(press); i++ {
		sums[i+1] = sums[i] + press[i]
	}
	// fmt.Println(sums)
	...
	// 	// 先頭が0からスタートするとき
	// }
	for i := 0; i < len(press); i += 2 {
		if 2*k+1+i < len(sums) {
			temp := sums[2*k+1+i] - sums[i]
			// fmt.Println(temp)
			ChMax(&ans, temp)
		}
	}
