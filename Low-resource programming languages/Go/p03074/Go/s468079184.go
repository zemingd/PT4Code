...
func solve(n, k int, s string) int {
	...
	for i := 0; i < len(s); i++ {
		if prev == s[i] && i > 0 {
			...
			if s[i-1] == '0' {
				byFoot = append(byFoot, len(blocks))
			}
			blocks = append(blocks, block{counter[i-1], s[i-1]})
			counter[i] = 1
		}
		if i == len(s)-1 {
			if s[i] == '0' {
			...
			blocks = append(blocks, block{counter[i], s[i]})
		}
		prev = s[i]
	}
	DEBUG = false
	...
	chosen := make([]bool, len(blocks))
	sumSeq := 0
	for i := 0; i < k; i++ {
		debug(blocks)
		maxSeq := 0
		maxIndex := 0
		for j := 0; j < len(byFoot); j++ {
			seq := 0
			index := byFoot[j]
			if chosen[index] {
				continue
			...
			if index != 0 {
				seq += blocks[index-1].seq
			}
			seq += blocks[index].seq
		...
		blocks[maxIndex].seq = sumSeq
		if maxIndex > 0 {
			blocks[maxIndex-1].seq = sumSeq
		}
		if maxIndex+1 < len(blocks) {
			blocks[maxIndex+1].seq = sumSeq
		}
	}
	debug(chosen)
