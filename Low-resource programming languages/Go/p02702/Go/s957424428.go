...
func main() {
	...
	// ss := make([]int, len(s))
	// for i, str := range s {
	// 	intStr, _ := strconv.Atoi(string(str))
	// 	ss[i] = intStr
	// }
	// fmt.Println(ss)
	//
	// cumsum := CumSum(ss)
	// fmt.Println(cumsum)
	fmt.Println(isMultiple(s))
}

var cache = map[string]int{}

func isMultiple(s string) int {
	if v, ok := cache[s]; ok {
		return v
	}
	...
	tempList := []int{0}
	for i := 0; i < len(s); i++ {
		intStr, _ := strconv.Atoi(string(s[i]))
		temp = (temp*10 + intStr) % m
