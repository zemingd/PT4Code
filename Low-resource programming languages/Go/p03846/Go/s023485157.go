...
func main() {
	...
	flag := true
	if N % 2 == 0 {
		for _, cnt := range absList {
			if cnt != 2 {
				flag = false
			}
		}
	} else {
		if _, exists := absList[0]; !exists {
			flag = false
		}
		for absValue, cnt := range absList {
			if (absValue == 0 && cnt != 1) || (absValue != 0 && cnt != 2) {
				flag = false
			}
		}
	}
