...
func main() {
	...
	sk := [][]int{}
	var lr []int
	h = append(h, 0) // stopping record
	// push
	sk = append(sk, []int{0, int(n)})
	for sz := len(sk); sz > 0; sz = len(sk) {
		// pop
		sk, lr = sk[:sz-1], sk[sz-1]
		l, r := lr[0], lr[1]
		// grow flowers from l to r.
		mn := lm.min(h[l:r]...)
		for i := l; i < r; i++ {
			h[i] -= mn
		}
		...
		// there is/are 0-height flower(s)
		// so split by 0-height flower and process next ranges
		nl := -1
		for i, v := range h {
			if nl == -1 && v > 0 {
				nl = i
