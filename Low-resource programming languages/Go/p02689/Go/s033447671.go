...
func main() {
	...
	observatories := io.ScanInts(n)
	highestObservatoryMap := map[int]int{}
	for i := 0; i < m; i++ {
		a, b := io.ScanInt(), io.ScanInt()
		if observatories[b-1] > highestObservatoryMap[a] {
			highestObservatoryMap[a] = observatories[b-1]
		}
		if observatories[a-1] > highestObservatoryMap[b] {
			highestObservatoryMap[b] = observatories[a-1]
		}
	}

	var goodObservatoryNum int
	for i, o := range observatories {
		if o > highestObservatoryMap[i+1] {
			goodObservatoryNum++
		}
	}
	fmt.Println(goodObservatoryNum)
}
