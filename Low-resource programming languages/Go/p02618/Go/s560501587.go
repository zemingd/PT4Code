...
func solve() {
	...
	for i := 0; i < D; i++ {
		...
		// maxlosspos := 0
		// for k := 0; k < 26; k++ {
		// 	if maxloss < c[k]*(i-last[k]) {
		// 		maxloss = c[k]*(i-last[k])
		// 		maxlosspos = k
		// 	}
		// }
		...
		// if maxloss > maxchange {
		// 	candidate = maxlosspos
		// }
	...
	for {
		...
		changeday := generator.Intn(D)
		changecontest := generator.Intn(26)
		if changecontest == tmp[changeday] {
			continue
		}
		tmp[changeday] = changecontest

		tmpscore := calc(tmp[0:D])

		if tmpscore > score {
			Logf("score %v -> %v, ans[%v]: %v -> %v\n", score, tmpscore,  changeday, ans[changeday], changecontest)
			ans[changeday] = changecontest
			score = tmpscore
		} else {
			continue
		}
