...
func main() {
	...
	spair := map[string]int{}
	mpair := map[string]int{}
	zero := 0
	for i := 0; i < N; i++ {
		sc.Scan()
		x, _ := strconv.ParseInt(sc.Text(), 10, 64)
		sc.Scan()
		y, _ := strconv.ParseInt(sc.Text(), 10, 64)
		if x == 0 && y == 0 {
			zero++
			continue
		}
	...
	for k, v := range spair {
		vp := ModPow(2, int64(v))
		if mpair[k] > 0 {
			pair := ModAdd(vp, ModPow(2, int64(mpair[k])))
			pair = ModSub(pair, 1)
			ans = ModMul(ans, pair)
			delete(mpair, k)
		} else {
