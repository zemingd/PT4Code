...
func solve() {
	...
	for i := 0; i < N; i++ {
		...
		if (A[i] > 0 && B[i] > 0) || (A[i] < 0 && B[i] < 0) {
			tiltA[pair{abs(A[i])/gcd, abs(B[i])/gcd}]++
		} else {
			// Logf("gcd %d\n", gcd)
			tiltB[pair{abs(A[i])/gcd, -abs(B[i])/gcd}]++
		}
	}
	...
	ans := 1
	for pr, cnt := range tiltA {
		tiltBcnt := tiltB[pair{pr.y, -pr.x}]
		ans = ans * (power2mod(cnt, magic) + power2mod(tiltBcnt, magic) - 1) % magic
		delete(tiltB, pair{pr.y, -pr.x})
	}

	for _, cnt := range tiltB {
		ans = ans * (power2mod(cnt, magic)) % magic
	}

	ans = ans * (power2mod(tilt10, magic) + power2mod(tilt01, magic) - 1) % magic

	fmt.Printf("%v\n", (ans + count0 - 1 + magic) % magic)
	// for i := 0; i < 100; i++ {
	// 	Logf("%d %v\n", i, power2mod(i, magic))
	// }
