...
func Solve() {
  ...
  for i := 0; i < N; i++ {
    ...
    if a == 0 && b == 0 {
      ...
      gcd = GCD(AbsInt(a), AbsInt(b))
      a, b = AbsInt(a) / gcd, AbsInt(b) / gcd
      if _, ok = MA[a]; !ok { MA[a] = make(map[int]int) }
      if _, ok = MA[a][b]; !ok { MA[a][b] = 0 }
      MA[a][b]++
    } else {
      gcd = GCD(AbsInt(a), AbsInt(b))
      a, b = AbsInt(a) / gcd, AbsInt(b) / gcd
      if _, ok = MB[a]; !ok { MB[a] = make(map[int]int) }
      if _, ok = MB[a][b]; !ok { MB[a][b] = 0 }
      MB[a][b]++
    }
  }
  cnt := 1
  ...
  for a, M = range MA {
    for b, v = range M {
      if _, ok = MB[b][a]; ok {
        cnt = cnt * (Pow(2, v) + Pow(2, MB[b][a]) - 1) % p
      } else {
        cnt = cnt * Pow(2, v) % p
      }
    }
  ...
  ans := (F[0] + (cnt * (Pow(2, F[1]) + Pow(2, F[2]) - 1)) % p - 1) % p
  Write(ans)
}
