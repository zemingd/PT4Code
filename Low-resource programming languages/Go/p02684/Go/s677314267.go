...
func main() {
	...
	for i := 0; i < len(lp); i++ {
		if lpstart == lp[i] {
			lpstartindex = int64(i)
			break
		}
	}
	//fmt.Println(lpstart, lpstartindex)
	// lprange := int64(len(lp)) - lpstartindex
	//fmt.Println(lp)
	nlpranges := lp[0:lpstartindex]
	lpranges := lp[lpstartindex:]
	if K < int64(len(nlpranges)) {
		fmt.Println(nlpranges[K-1])
	} else {
		K -= int64(len(nlpranges))
		fmt.Println(lpranges[K%int64(len(lpranges))] + 1)
	}
	// fmt.Println("nlpranges", nlpranges)
	// fmt.Println("lpranges", lpranges)
	// fmt.Println(lpranges[K%int64(len(lpranges))] + 1)

	//fmt.Println(lp[K%int64(len(lp))] + 1)
}
