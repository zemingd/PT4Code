...
func main() {
	...
	foundTownMap := map[int]int{}
	curTown := 1
	curCount := 0
	for i := 0; i < k; i++ {
		curTownIndex := curTown-1
		if i == k {
			return
		}
		if foundTownMap[curTown] > 0 {
			curCount = i
			break
		}
		foundTownMap[curTown] = i
		curTown = towns[curTownIndex]
	}
	loopNum := curCount-foundTownMap[curTown]
	remain := (k-curCount)%loopNum
	for i := 0; i < remain; i++ {
		curTown = towns[curTown-1]
	}
	fmt.Println(curTown)
}
