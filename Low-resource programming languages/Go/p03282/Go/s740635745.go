...
func main() {
	...
	for i := 0; i < len(S); i++ {
		if i == 0 {
			prev = S[0]
			tmpstring = string(S[0])
			continue
		}
		if S[i] == prev {
			tmpstring = tmpstring + string(S[i])
			prev = S[i]
			continue
		}
		group = append(group, tmpstring)
...
/**
  5000兆日後に何文字増えるか
*/
func calc(a string) (string, int64) {
	len := int64(len(a))
	number := s2i(string(a[0]))
	if number == 1 {
