...
func solve() {
	a, b, c, d, e, f := ri(), ri(), ri(), ri(), ri(), ri()

	s, t := -1, -1
	for i := 0; a*i*100 <= f; i++ {
		for j := 0; a*i*100+b*j*100 <= f; j++ {
			if (a*i*100+b*j*100)*(100+e) <= f*100 {
				s, t = i, j
			}
		}
	}
	water := a*s*100 + b*t*100

	sugar := 0
	for i := 0; i*c+water <= f; i++ {
		for j := 0; i*c+j*d+water <= f; j++ {
			if (c*i+d*j)*100 <= water*e {
				chmax(&sugar, c*i+d*j)
			}
		}
	}
