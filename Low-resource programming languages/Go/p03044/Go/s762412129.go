...
func solve(n int, edges []Edge) {
	...
	for _, edge := range edges {
		...
		if colors[edge.u] == init && colors[edge.v] == init {
			colors[edge.u] = 0
		}
		if edge.w%2 == 0 {
			...
			if colors[edge.u] == init {
				...
				panic(fmt.Sprintf("unexpected state colors %v %d %d", colors, edge.u, edge.v))
			}
		} else {
			if colors[edge.u] == init {
		...
		// 	if colors[edge.u] == init {
		// 		nextEdges = append(nextEdges, edgeMap[edge.u])
		// 	}
		// 	if !used[edge.v] {
		// 		nextEdges = append(nextEdges, edgeMap[edge.v])
		// 	}
		// }
		// if len(nextEdges) == 0 {
		// 	break
