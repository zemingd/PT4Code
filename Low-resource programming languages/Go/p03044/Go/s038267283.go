...
func main() {
	...
	edges := make([]Edge, n-1)
	for i := 0; i < n-1; i++ {
		edges[i] = Edge{readi() - 1, readi() - 1, readi()}
	}
	solveByDfs(n, edges)
}
...
func solveByDfs(n int, edges []Edge) {
	edgeMap := make(map[int][]Edge)
	for i := 0; i < n-1; i++ {
		edgeMap[edges[i].u] = append(edgeMap[edges[i].u], edges[i])
		edgeMap[edges[i].v] = append(edgeMap[edges[i].v], edges[i])
	}
	distList = make([]int, n)
	dfs(-1, 0, 0, edgeMap)
	for i := 0; i < n; i++ {
...
func dfs(prev, current, dist int, edgeMap map[int][]Edge) {
	for _, e := range edgeMap[current] {
		next := -1
		if e.u == current {
			next = e.v
		} else {
		...
		distList[next] = dist + e.w
		dfs(current, next, dist+e.w, edgeMap)
	}
}

func solve(n int, edges []Edge) {
	edgeMap := make(map[int]Edge)
	for i := 0; i < n-1; i++ {
		edgeMap[edges[i].u] = edges[i]
		edgeMap[edges[i].v] = edges[i]
	}
	colors := make([]int, n)
	init := -1
	...
	for {
		for _, edge := range edges {
			if edge.w%2 == 0 {
				// u and v are same color
				if colors[edge.u] == init {
					colors[edge.u] = colors[edge.v]
				} else if colors[edge.v] == init {
					colors[edge.v] = colors[edge.u]
				}
			} else {
				if colors[edge.u] == init {
					colors[edge.u] = reverse(colors[edge.v])
				} else if colors[edge.v] == init {
					colors[edge.v] = reverse(colors[edge.u])
				}
			}
		}
		if finished(colors) {
