...
func (p1 Vertex) Less(p2 interface{}) bool {
	return p1.distance < p2.(Vertex).distance
}
...
func main() {
	...
	for i := 0; i < N-1; i++ {
		...
		u--
		v--
		edges[u] = append(edges[u], Vertex{id: v, distance: w})
		edges[v] = append(edges[v], Vertex{id: u, distance: w})
	}

	q := NewPriorityQueue()
	q.Push(Vertex{id: 0, distance: 0})
	for q.Len() > 0 {
		v := q.Pop().(Vertex)
		if distance[v.id] != -1 {
			continue
		...
		for _, edge := range edges[v.id] {
			q.Push(Vertex{edge.id, edge.distance + v.distance})
		}
	}
	//printf("%#v\n", edges)
