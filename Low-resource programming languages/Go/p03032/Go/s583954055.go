...
func solve(io *Io, d *Io) {
	...
	K := io.NextInt()
	rawVs := io.NextInts(N)
	vs := append(rawVs, rawVs...) // 連結することで「左右から合計 N 個取得する」を「連結後の配列から N 個連続で取得する」で表現する
	...
	for dequeueCount := 0; dequeueCount <= maxDequeueCount; dequeueCount++ {
		...
		// Dequeue dequeueCount items from index i
		for i := 0; i < dequeueCount+1; i++ {
			dequeued := vs[N-i : N-i+dequeueCount]
			dequeued = append([]int{}, dequeued...) // to keep vs immutable
			sort.Ints(dequeued)
