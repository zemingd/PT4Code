...
func (l byKey) Less(i, j int) bool {
	return l.CardList[i].key < l.CardList[j].key
}

// how to use
// L := make(CardList, 0, 200000+5)
// L = append(L, &Card{key: intValue})
...
func main() {
	...
	for i := 0; i < m; i++ {
		b, c := B[i], C[i]
		L = append(L, &Card{upper: b, val: c})
	}
	sort.Stable(sort.Reverse(L))

	cur := 0
OUTER:
	for _, c := range L {
		upper, val := c.upper, c.val
		for i := 0; i < upper; i++ {
			if A[cur] < val {
				A[cur] = val
				cur++
