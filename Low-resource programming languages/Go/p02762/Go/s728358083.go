...
func (u *UnionFind) belongsToSameTree(a, b int) bool {
...
func solve() {
	...
	blockLists := map[int][]int{}
	for range make([]int, K) {
		a := io.nextInt() - 1
		b := io.nextInt() - 1
		blockLists[a] = append(blockLists[a], b)
		blockLists[b] = append(blockLists[b], a)
	}

	for i := range make([]int, N) {
		candidates := 0
		treeSize := -union.data[union.findRoot(i)]
		candidates = treeSize - 1 - friends[i] // except themselves and frends
		for _, block := range blockLists[i] {
			if union.belongsToSameTree(i, block) {
				candidates--
			}
		}
