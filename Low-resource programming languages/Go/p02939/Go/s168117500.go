...
func solve2(io *Io, d *Io) {
	...
	// dp[i][j]: i ~ i+j の連続文字を最後に使用したケースの最大個数
	dp := make([][]int, len(S))
	for i := range dp {
		dp[i] = make([]int, 4)
	}
	...
	for i := 1; i < len(chs); i++ {
		for k := 0; k < 4 && i+k < len(chs); k++ {
			for prevK := 0; prevK < 4 && i-prevK-1 >= 0; prevK++ {
				if dp[i-prevK-1][k] == 0 {
					// 満たす文字列が存在しない
					continue
				}
				...
				prev := string(chs[i-prevK-1 : i])
				curr := string(chs[i : i+k+1])
				if prevK != k || prev != curr {
					dp[i][k] = Max(dp[i][k], dp[i-prevK-1][prevK]+1)
				}
			}
		}
	}
