...
func main() {
	...
	log.Println(wCnt, eCnt)
	rtn := N
	for i := 0; i < N; i++ {
		rtn = min(rtn, wCnt[i]+eCnt[i])
	}
	fmt.Println(rtn)
}

func cntWest(east []bool) int {
	return len(east) - cntEast(east)
}

func cntEast(east []bool) int {
	rtn := 0
	for _, b := range east {
		if b {
			rtn++
...
func newScanner() *scanner {
	rdr := bufio.NewReaderSize(os.Stdin, 100000)
