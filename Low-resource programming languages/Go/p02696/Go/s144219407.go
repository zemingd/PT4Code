...
func main() {
	...
	/* --- code --- */
	A, B, N := nextInt64(), nextInt64(), nextInt64()
	// A, B, N := int64(5), int64(7), int64(4)

	max_score := int64(0)
	if N <= B {
		max_score = calc(A, B, N)
	} else {
		max_score = calc(A, B, B-1)
		for x := B*2 - 1; x <= N; x += B {
			score := calc(A, B, x)
			if max_score < score {
				max_score = score
...
func calc(A int64, B int64, x int64) int64 {
	left := int64((A * x) / B)
	right := A * int64(x/B)
	return left - right
	// return int64((A*x)/B) - A*int64(x/B)
}
