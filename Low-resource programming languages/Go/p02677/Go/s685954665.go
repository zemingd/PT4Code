...
func main() {
	...
	M := io.NextInt()
	Logf("%v\n", M)
	hradi := (float64(H)/float64(12)+float64(M)/float64(720))*2.0*3.14159265358979323846264338327950288419716939937510582097494459
	mradi := float64(M)/float64(60)*2.0*3.14159265358979323846264338327950288419716939937510582097494459
	Logf("%v %v %v\n", hradi, math.Sin(hradi), math.Cos(hradi))
	Logf("%v\n", mradi)
	ax:=math.Sin(hradi)*float64(A)
	ay:=math.Cos(hradi)*float64(A)
	Logf("%v %v\n", ax, ay)
	bx:=math.Sin(mradi)*float64(B)
	by:=math.Cos(mradi)*float64(B)
	Logf("%v %v\n", bx, by)
