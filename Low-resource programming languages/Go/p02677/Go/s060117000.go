...
func main() {
	...
	A, B, H, M := readInt(), readInt(), readInt(), readInt()
	hrad := H*30 + int64(0.5*float64(M))
	mrad := M * 6
	var kakudo float64
	var rad float64
	if abs(int64(hrad-mrad)) > 180 {
		kakudo = float64(360 - abs(hrad-mrad))
	} else {
		kakudo = float64(abs(hrad - mrad))
	}
	rad = kakudo * math.Pi / 180
	s := fmt.Sprintf("%.12f", math.Sqrt(float64(A*A+B*B)-float64(2.0*A*B)*math.Cos(rad)))
	fmt.Println(s)
}
