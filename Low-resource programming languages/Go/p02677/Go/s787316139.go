...
func main() {
	...
	thetaA := float64(360 * (H*60+M) / (12*60)) * math.Pi / 180
	thetaB := float64(360 * M / 60) * math.Pi / 180

	//fmt.Println(float64(H * 30), float64(M * 6))

	posA := []float64{float64(A) * math.Cos(thetaA), float64(A) * math.Sin(thetaA)}
	posB := []float64{float64(B) * math.Cos(thetaB), float64(B) * math.Sin(thetaB)}

	//fmt.Println(posA, posB)

	distance := math.Sqrt(math.Pow(posA[0] - posB[0], 2) + math.Pow(posA[1] - posB[1], 2))
