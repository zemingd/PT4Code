...
func main() {
	sc := NewScanner()
	A, B, H, M := sc.NextFloat(), sc.NextFloat(), sc.NextFloat(), sc.NextFloat()

	L := H/12.0*360 + M/60.0*30
	S := M / 60 * 360

	x1 := A * math.Sin(L/180*math.Pi)
	y1 := A * math.Cos(L/180*math.Pi)

	x2 := B * math.Sin(S/180*math.Pi)
	y2 := B * math.Cos(S/180*math.Pi)
