...
func main() {
	...
	prefixBuffer := make([]string, 10e5)
	suffixBuffer := make([]string, 10e5)
	S := readString()
	Q := readInt()
	rv := int64(0)
	for i := int64(0); i < Q; i++ {
		...
		if q == 1 {
			...
			if f == 1 {
				if rv%2 == 0 {
					//先頭
					prefixBuffer = append(prefixBuffer, c)
				} else {
					// 末尾
					suffixBuffer = append(suffixBuffer, c)
				}
			} else {
				if rv%2 == 0 {
					// 末尾
	...
	// 先頭に追加しているのでリバースして上げる必要がある
	prefixBuffer = reverseStr(prefixBuffer)
	// 文字列を構築する
	s := strings.Join(prefixBuffer, "") + S + strings.Join(suffixBuffer, "")
	if rv%2 == 1 {
		t := make([]string, len(s))
		for i := len(s) - 1; i >= 0; i-- {
