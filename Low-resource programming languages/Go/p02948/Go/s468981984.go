...
func NewPq(c int) Pq { ; p := make(Pq, 1, c); p[0] = pqe{v: -1, p: -1}; return p }
func (pq *Pq) Push(v int, p int) {
	*pq = append(*pq, pqe{v, p})
	for i := len(*pq) - 1; i > 1 && (*pq)[i].p > (*pq)[i/2].p; i = i / 2 {
...
func (pq *Pq) Pop() interface{} {
	...
	*pq = (*pq)[:len(*pq)-1]
	for i := 1; ; {
		lc, rc := 2*i, 2*i+1
		var lrg int
		if lc < len(*pq) && (*pq)[lc].p > (*pq)[i].p {
			lrg = lc
		} else {
			lrg = i
		}
		if rc < len(*pq) && (*pq)[rc].p > (*pq)[lrg].p {
			lrg = rc
		}
		if lrg == i {
			break
		}
		(*pq)[i], (*pq)[lrg] = (*pq)[lrg], (*pq)[i]
		i = lrg
	}
	return me.v
}
...
func (pq *Pq) Clear()        { *pq = (*pq)[:1] }
func (pq *Pq) Size() int     { return len(*pq) - 1 }
func (pq *Pq) IsEmpty() bool { return pq.Size() == 0 }

func solve() {
	n, m := ri(), ri()
	as, bs := riis(n)
	const aMax = int(10)
	works := make([][]int, aMax+1)
	for i := 0; i < n; i++ {
		intsAdd(&works[as[i]], bs[i])
	}
	ans := 0
	pq := NewPq(n)
	for i := 1; i <= m; i++ {
		for _, v := range works[i] {
			pq.Push(v, v)
		}
		if get, ok := pq.Pop().(int); ok {
			ans += get
		}
	}
	pln(ans)
