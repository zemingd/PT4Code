...
// 	fmt.Println(field)
	...
	for len(startQue) > 0 {
		...
		for len(que) > 0 {
			var value me
			value, que = shift(que)
			if scoreMap[value.h][value.w] == 1 && scores[value.h][value.w] <= value.count{
				continue
			}
			scoreMap[value.h][value.w] = 1
			scores[value.h][value.w] = value.count
			...
			for i:= -2; i<=2; i++ {
				for j:= -2; j<=2; j++ {
					...
					if (value.h+i < 0) || h <= value.h+i || 0 > value.w+j || w <= value.w+j  {
						continue
					}
					if field[value.h+i][value.w+j] == "#" {
						continue
					}
					startQue = append(startQue, me{value.h+i, value.w+j, value.count + 1, "left2"})
				}
			}
			// left2
