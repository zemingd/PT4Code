...
func main() {
	...
	for len(queue) > 0 {
		q := queue[0]
		//fmt.Println(filled, "POS", q.x, q.y)
		queue = queue[1:]
		var ok = false
		for _, p := range []*P{
			{
			...
			if _, e := filled[fmt.Sprintf("%d/%d", p.x, p.y)]; e {
				if filled[fmt.Sprintf("%d/%d", p.x, p.y)] > p.times {
					filled[fmt.Sprintf("%d/%d", p.x, p.y)] = p.times
				}
				continue
			}
			if D[p.y][p.x] == "." {
		...
		//fmt.Println("CUR", p.x, p.y)
		if !ok {
			for _, p := range []*P{
				{
					x: q.x + 2, y: q.y - 2, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 2, y: q.y - 1, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 2, y: q.y, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 2, y: q.y + 1, score: q.score, times: q.times + 1,
				},
				...
				{
					x: q.x + 1, y: q.y - 2, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 1, y: q.y - 1, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 1, y: q.y, score: q.score, times: q.times + 1,
				},
				{
					x: q.x + 1, y: q.y + 1, score: q.score, times: q.times + 1,
				},
				...
				{
					x: q.x, y: q.y - 2, score: q.score, times: q.times + 1,
				},
				{
					x: q.x, y: q.y - 1, score: q.score, times: q.times + 1,
				},
				{
					x: q.x, y: q.y, score: q.score, times: q.times + 1,
				},
				{
					x: q.x, y: q.y + 1, score: q.score, times: q.times + 1,
				},
	...
	//fmt.Println(filled)
	if v, e := filled[fmt.Sprintf("%d/%d", DW, DH)]; e {
		fmt.Println(v)
	} else {
		fmt.Println(-1)
	}
