...
func main() {
	...
	for len(queue) > 0 {
		...
		costs[fmt.Sprintf("%d/%d", q.x, q.y)] = 0
		for _, p := range []*P{
			{
	...
	for len(visited) > 0 {
		...
		for xx := -2; xx <= 2; xx++ {
			for yy := -2; yy <= 2; yy++ {
				...
				cur := fmt.Sprintf("%d/%d", q.x, q.y)
				nx, ny := q.x+xx, q.y+yy
				next := fmt.Sprintf("%d/%d", nx, ny)
				...
				} else if costs[cur]+1 < costs[next] {
					costs[next] = costs[cur] + 1
				}
			}
		}
	}
	if v, e := costs[fmt.Sprintf("%d/%d", DW, DH)]; e {
		fmt.Println(v)
	} else {
		fmt.Println(-1)
	}
