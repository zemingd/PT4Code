		...
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
				if m[i][j] != '#' && m[i][j] != '.' {
					if m[i][j] == '.' {
						continue
						// panic(fmt.Sprintf("unexpected state at %d %d", i, j))
					}
					updated = updated || move2(i, j, m[i][j])
				}
			}
		}
		if updated {
