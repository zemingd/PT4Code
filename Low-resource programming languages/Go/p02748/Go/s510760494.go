...
func main() {
	...
	a := make([]int64, A)
	b := make([]int64, B)
	x, y, c := make([]int64, M), make([]int64, M), make([]int64, M)
	for i := int64(0); i < A; i++ {
		a[i] = readInt()
	}
	for i := int64(0); i < B; i++ {
	...
	for i := int64(0); i < M; i++ {
		x[i], y[i], c[i] = readInt()-1, readInt()-1, readInt()
	}
	// 最小パターン1 a,bのどちらの最小値を足すパターン
	ans := int64(INF)
	ans = min(a...) + min(b...)
	...
	// 最小パターン2 割引を使用した最小値
	for i := int64(0); i < M; i++ {
		ans2 = min(ans2, a[x[i]]+b[y[i]]-c[i])
	}
	fmt.Println(min(ans, ans2))
}
