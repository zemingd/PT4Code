...
arr[1..-1].each do |i|
  ...
  arr =[]
  if common_elms.size < crr_elms.size
    common_elms.each do |i|
      ind = crr_elms.bsearch { |x| x >= i}
      break unless ind
      arr << i if crr_elms[ind] == i
    end
    common_elms = arr
  else
    crr_elms.each do |i|
      ind = common_elms.bsearch { |x| x >= i }
      break unless ind
      arr << i if common_elms[ind] == i
    end
    common_elms = arr
  end
  if pair
    ...
    break set = false if !common_elms.empty?
  end
end
