...
i = 0
while i <= count - 1
  if overlap_e.count > 0 && overlap_e.include?(dic_sort[i])
    overlap_list = restaurant_list.select{|re| re[0] == dic_sort[i]}.sort {|a, b| b[1] <=> a[1] }
    overlap_list.each do |overlap|
      dic_sort_index.push(restaurant_list.index(overlap) + 1) if restaurant_list.index(overlap) != nil
    end
    i += overlap_list.count
  else
    dic_sort_index.push(restaurant_names.index(dic_sort[i]) + 1) if restaurant_names.index(dic_sort[i]) != nil
    i += 1
  end 
end 
