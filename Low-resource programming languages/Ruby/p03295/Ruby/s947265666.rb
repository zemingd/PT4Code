...
while ! finished?(last_fallen_edge, lefts)
    ...
    bench= Benchmark.realtime { rest_edges = rest_edges.find_all {|e| e.left >= last_fallen_edge} }
  warn "./Main.rb:43: warning: find_all(#{edge_num}): #{bench}" 
    # puts rest_edges.size
    bench= Benchmark.realtime {last_fallen_edge = rest_edges.sort_by {|e| e.right }[0].right}
  warn "sort (#{edge_num}): #{bench}" if bench > 0.2
