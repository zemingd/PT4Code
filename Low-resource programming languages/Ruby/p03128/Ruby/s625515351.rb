...
num_sticks.delete(1) if num_sticks.include?(4)
num_sticks.delete(2) if num_sticks.include?(3) || num_sticks.include?(5)
num_sticks.delete(3) if num_sticks.include?(5)
num_sticks.delete(6) if num_sticks.include?(7) || num_sticks.include?(9)
num_sticks.delete(9) if num_sticks.include?(7)
num_sticks.sort! { |a, b| (a[1] <=> b[1]).nonzero? || b[0] <=> a[0] }
selected_nums = Hash.new(0)
reduced_sticks = [(n - 22), 0].max
