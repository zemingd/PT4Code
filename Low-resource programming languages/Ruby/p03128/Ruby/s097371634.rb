...
def pack_with_all_combi(n, hons)
  #puts "#{[n, hons]}"
  if hons.length == 6 then s = 1
  elsif hons.length == 5 then s = 2
  elsif hons.length == 4 then s = 2
  elsif hons.length == 3 then s = 2
  elsif hons.length == 2 then s = 1
  else                        s = 0
  end
  ansA = []
  if hons.length >= 2
    (1..s).each do |i|
      hons[1..-1].combination(i).each do |part_hons|
        ansA.push pack(n, [hons[0]] + part_hons)
      end
    end
  else
    ansA.push pack(n, hons)
  end
