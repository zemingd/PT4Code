...
start_pos.each do |start_y, start_x|
  ...
  costs = Array.new(h).map { Array.new(w, 10000) }
  ...
  while !queue.empty?
    ...
    4.times do |i|
      ...
      # next if nodes[ny][nx] != '.' && costs[ny][nx] <= node.cost
      next if costs[ny][nx] <= node.cost
      queue.unshift(Node.new(node.cost + 1, ny, nx))
    end
  end
  tmp_cost = costs.map { |row| row.max }.max
  max_cost = [max_cost, tmp_cost].max
  # debug(costs)
  # puts "----"
