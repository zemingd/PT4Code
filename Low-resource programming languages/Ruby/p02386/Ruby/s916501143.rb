class Dice
  ...
  def ==(other)
    c = @n.uniq.map{|e| [e, @n.count(e)]}.sort{|a,b|a[1]<=>b[1]}
    other.set_upper_by_value(c[0][0])
    return true if c.size < 2 # only 1 number
    idx = @n.map.with_index{|p,i| i if p==c[0][0]}.compact
    idx.each{|i|
